...
while True :
    ...
    if int(bal) > 0 :
        ...
        treetxid = source_code.xpath(xpatch_txid)
        xVol = str(treetxid[0].text_content())
        print(Back.WHITE , Fore.GREEN , 'SCAN No:' , str(count) , ' Win No:' , str(win) , ' Balance:' , str(xVol))
        print(Back.GREEN , Fore.WHITE , 'ADDRESS==> ' , str(address))
        print(Back.WHITE , Fore.GREEN , str(private_key))
        f = open(u"winner11.txt" , "a")
        f.write('\nBitcoin Address Compressed : ' + address + '  BAL= ' + str(xVol))
        f.write('\nPassphrase       : ' + passphrase)
        f.write('\nPrivate Key      : ' + private_key)
        f.close()
    else :
        print(Back.RED , Fore.WHITE , 'Scan:' , str(count) , ' WiN:' , str(win) , Style.RESET_ALL)
        print(Back.RED , Fore.WHITE , str(address) , '     [TXID]:' , str(bal) , Style.RESET_ALL)
        print(Back.WHITE , Fore.RED , str(private_key) , Style.RESET_ALL)
