...
for i in range(0 , len(mylist)) :
    ...
    txid = xBal(address)
    ifbtc = '0 BTC'
    print(Back.RED , Fore.WHITE , 'SCAN:' , str(z) , '  WIN:' , str(w) , ' WithVol:' , str(s))
    print(Back.WHITE , Fore.RED , str(address) , '           TXiD No:' , str(txid))
    print(Back.RED , Fore.WHITE , str(private_key))
    z += 1
    if int(txid) > 0 :
        w += 1
        urlblock = "https://bitcoin.atomicwallet.io/address/" + address
        ...
        print(Back.YELLOW , Fore.BLACK , 'SCAN:' , str(z) , '  WIN:' , str(w) , ' WithVol:' , str(s) ,
              '  [+] Balance:' , str(xVol))
        print(Back.WHITE , Fore.BLACK , str(address) , '           TXiD No:' , str(txid))
        print(Back.YELLOW , Fore.BLACK , str(private_key))
